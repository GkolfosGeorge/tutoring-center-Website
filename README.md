...
# Δημιουργία αρχικού λογαριασμού admin
npx prisma db seed
...
Το `npx prisma db seed` δημιουργεί (ή επαναφέρει) τον λογαριασμό `admin` με **τυχαίο κωδικό που τυπώνεται μία φορά στο τερματικό** — δεν αποθηκεύεται πουθενά αλλού. Σημείωσέ τον αμέσως. Αν θέλεις συγκεκριμένο κωδικό, όρισε `SEED_ADMIN_PASSWORD` στο `.env` πριν το τρέξεις.
